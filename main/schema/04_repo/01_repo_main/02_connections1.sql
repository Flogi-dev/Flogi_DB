...
CREATE TABLE repo_connections (
    repo_id UUID PRIMARY KEY REFERENCES repo_main(repo_id) ON DELETE CASCADE, -- repo_main 테이블의 저장소 ID (PK, FK) [기존 repo_uuid에서 변경]
    ...
    webhook_uuid_on_platform TEXT,                  -- VCS 플랫폼에 등록된 Webhook의 UUID (Webhook을 사용하는 경우) [기존 webhook_id_on_platform에서 변경]
    access_token_ref_uuid TEXT,                     -- 이 저장소 접근에 사용되는 Access Token의 외부 저장소 참조 UUID 또는 Comfort Commit 내부 토큰 관리 시스템의 식별자. 실제 토큰 값은 저장하지 않음. [기존 access_token_ref_id에서 변경]
    ...
    comfort_commit_config_json JSONB DEFAULT '{}'::JSONB, -- 해당 저장소에 대한 Comfort Commit 서비스의 특화 설정 
                                                          -- 예: {"auto_analysis_enabled": true, "default_llm_model_for_repo": "gpt-4o", "commit_style_template_uuid": "uuid", "analysis_branch_filter": ["main", "develop"]}
...
-- 인덱스
CREATE INDEX idx_repo_connections_status ON repo_connections(connection_status); -- 특정 연결 상태의 저장소 조회 (기존 uuidx_ 접두사에서 표준 idx_로 변경)
CREATE INDEX idx_repo_connections_access_token_ref ON repo_connections(access_token_ref_uuid) WHERE access_token_ref_uuid IS NOT NULL; -- 컬럼명 변경 반영 (기존 uuidx_ 접두사에서 표준 idx_로 변경)
CREATE INDEX idx_repo_connections_webhook_uuid ON repo_connections(webhook_uuid_on_platform) WHERE webhook_uuid_on_platform IS NOT NULL; -- 컬럼명 변경 반영 (기존 uuidx_ 접두사에서 표준 idx_로 변경)
